    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            for iteration, sample in enumerate(data_loader):
                ...
                loss = loss_fn(logits, sample['answer'].view(-1))
                acc = utils.accuracy(logits, sample['answer'].view(-1))
        ...
        if (logs['valid']['best_epoch'] + args.patience == epoch ) or (epoch == args.epochs-1):
            if not os.path.exists('bin'):
                os.mkdir('bin')
            torch.save(logs['valid']['model'], 'bin/oracle.pt')
...
if __name__ == '__main__':
    ...
    # Hyperparameter
    parser.add_argument('-wed', '--word_embedding_dim', type=int, default=300)
    parser.add_argument('-ced', '--cat_embedding_dim', type=int, default=512)
    parser.add_argument('-hs', '--hidden_size', type=int, default=512)
    parser.add_argument('-mh', '--mlp_hidden', type=int, default=128)
